...
import type { TurnStatus } from './codex-client.js';
...
export interface CodexPoolOptions {
  onTurnCompleted?: (context: StreamingContext, status: TurnStatus) => void;
}
...
export class CodexPool {
  ...
  private onTurnCompletedCallback?: (context: StreamingContext, status: TurnStatus) => void;

  constructor(slack: WebClient, options?: CodexPoolOptions) {
    this.slack = slack;
    this.onTurnCompletedCallback = options?.onTurnCompleted;
  }
  ...
  private async createRuntime(conversationKey: string): Promise<CodexRuntime> {
    ...
    // Register turn completion callback to clear busy state
    if (this.onTurnCompletedCallback) {
      streaming.onTurnCompleted((context, status) => {
        this.onTurnCompletedCallback!(context, status);
      });
    }
