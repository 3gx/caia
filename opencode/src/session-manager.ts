...
import type { ActivityEntry as SharedActivityEntry } from 'caia-slack';

/**
 * SDK Permission Mode type - matches OpenCode semantics.
 * - 'plan': Read-only mode, no tools
 ...
 * Re-exported from the shared caia-slack package.
 */
export type { SharedActivityEntry as ActivityEntry };
...
const storeManager = new SessionStoreManager<Session, ThreadSession>('opencode');
...
/**
 * Delete session for a channel (session store only).
 * OpenCode session deletion should be performed via the SDK by the caller.
 *
