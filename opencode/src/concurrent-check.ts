 ...
 * ======================================================
 *
 * We need a robust way to detect if an OpenCode session is running
 * in a terminal. Current approaches that DON'T work on macOS:
 *
 * 1. `ps aux | grep "opencode session resume <sessionId>"`
 *    - macOS truncates command-line args in ps output
 *    - Only shows "opencode" without the resume argument
 *
 * 2. `lsof <session-file.jsonl>`
 *    - OpenCode doesn't keep a session file open continuously
 *    - Opens, writes, closes - so lsof finds nothing
 *
 * 3. Working directory matching via `lsof -d cwd`
 *    - Too broad - catches ANY OpenCode in same directory
 *    - False positives for unrelated OpenCode sessions
 *
 ...
 * - Whether OpenCode creates lock/PID files we can check
 * - OpenCode hooks or config that tracks active sessions
...
export async function isSessionActiveInTerminal(
  ...
  try {
    ...
    const sessionFile = `${process.env.HOME}/.opencode/projects/${projectPath}/${sessionId}.jsonl`;
      ...
      try {
        ...
        if (tty && tty !== '??' && tty !== '') {
          return {
            active: true,
            pid,
            command: `opencode --resume ${sessionId}`,
          };
        }
      } catch {
        continue;
