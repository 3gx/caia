...
import { getSession, saveSession, getThreadSession, saveThreadSession, getOrCreateThreadSession, saveMessageMapping, findForkPointMessageId, getActivityLog } from '../../../claude/src/session-manager.js';
import { isSessionActiveInTerminal } from '../../../claude/src/concurrent-check.js';
...
describe('slack-bot command handlers', () => {
  ...
  describe('/compact command with abort', () => {
    ...
    it('should add conversationKey to busyConversations during /compact', async () => {
      ...
      // Import conversationTracker to verify tracking
      ...
      vi.mocked(startClaudeQuery).mockReturnValue({
        [Symbol.asyncIterator]: async function* () {
          ...
          busyDuringCompact = conversationTracker.isBusy(sessionId);
          yield { type: 'system', subtype: 'compact_boundary', compact_metadata: { pre_tokens: 5000 } };
          yield { type: 'result', result: '' };
        },
        interrupt: vi.fn(),
  ...
  describe('busy conversation :eyes: reaction handling', () => {
    it('should remove :eyes: reaction when responding with busy message', async () => {
      ...
      // Import conversationTracker to set up busy state
      const { conversationTracker } = await import('../../../claude/src/slack-bot.js');
      const sessionId = 'existing-session-123';
